...
As mentioned earlier, we select the largest cp where xerror is within 1 standard error of the minimum (to balance simplicity and accuracy). based on this our best possible tree would have 33 nodes and complexity 
...
The below tree shows that our best cp = 0.0011 where the tree has 33 splits and cross validation error 0.3081
...
This si mainly driven by the fact that importance is measured by performance gain (e.g., reduction in MSE/Gini impurity).This captures how much a variable improves model accuracy but does not imply statistical significance. 

For example, in our graphs above, height and grip force have higher importance but not showing in the modle. this is because these 2 variables are more linked to variable like gender where males are taller and have stronger grip force but this confound with gender and since tree used gender then these are becoming redundant variables in tree build.
...
The above-pruned tree diagram is based on the 1-SE rule where we can see that the tree have 33 nodes and CP of 0.0017. Next, we plot the tree diagram based on the minimum cross-validation error
